...
export function runMeetingsBusinessTests() {
  assert(
    !validateMeetingsSubmission({
      requestedMeetings: 0,
  ...
  try {
    ...
    throw new Error("totalMeetings 0 should throw");
  } catch (error) {
    assert(error instanceof Error, "totalMeetings 0 should throw an Error");
  }
}
